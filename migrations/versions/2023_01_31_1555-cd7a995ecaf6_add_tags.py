...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.Column("flag_id", sa.INTEGER(), autoincrement=False, nullable=False),
        # sa.Column("user_id", sa.INTEGER(), autoincrement=False, nullable=False),
        sa.ForeignKeyConstraint(["flag_id"], ["tags.id"], name="tags_issues_link_fk"),
        sa.ForeignKeyConstraint(["issue_id"], ["issues.id"], name="tags_issues_link_fk_1"),
        sa.PrimaryKeyConstraint("issue_id", "flag_id", name="tags_issues_link_pkey"),
        schema=None,
    )
