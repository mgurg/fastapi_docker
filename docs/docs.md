...
CREATE TABLE files_ideas_link (
    idea_id int,
    file_id int,
    PRIMARY KEY(idea_id, file_id)
);

ALTER TABLE public.files_ideas_link ADD CONSTRAINT files_ideas_link_fk FOREIGN KEY (idea_id) REFERENCES public.ideas(id);
ALTER TABLE public.files_ideas_link ADD CONSTRAINT files_ideas_link_fk_1 FOREIGN KEY (file_id) REFERENCES public.files(id);
