...
def tags_add_one(*, db: Session = Depends(get_db), tag: TagCreateIn, auth_user: CurrentUser):
...
def tags_edit_one(*, db: Session = Depends(get_db), tag_uuid: UUID, tag: TagEditIn, auth_user: CurrentUser):
...
def tags_delete_one(
    *, db: Session = Depends(get_db), tag_uuid: UUID, force_delete: bool = False, auth_user: CurrentUser
):
    db_tag = crud_tags.get_tag_by_uuid(db, tag_uuid)
