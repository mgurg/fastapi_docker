...
def parse_non_recurring(data):
    ...
    start_date = pendulum.instance(data["date_from"])
    end_date = pendulum.instance(data["date_to"])
    temp_dict = {
        "uuid": uuid4(),
        "task_uuid": data["uuid"],
        "title": data["title"],
        ...
        "end": end_date.to_date_string(),
        "bgcolor": data["color"],
    }

    if data["all_day"] == False:
        temp_dict["time"] = start_date.to_time_string()
        temp_dict["duration"] = start_date.diff(end_date).in_minutes()
